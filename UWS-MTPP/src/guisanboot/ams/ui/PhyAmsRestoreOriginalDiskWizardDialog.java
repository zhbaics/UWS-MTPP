 ...
 */
public class PhyAmsRestoreOriginalDiskWizardDialog extends WizardDialogSample implements RecordUWSRptable,RestoreOrigiDiskable,RestoreOriginalDiskInterface{
    AmsActModePaneForRestWinOrgDisk actModePane;
    DefineRstRelationshipPane definePane;
    SelectAmsRestoreDiskPane selectRestDiskPane;
    ...
    public PhyAmsRestoreOriginalDiskWizardDialog(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
    }
    
    public PhyAmsRestoreOriginalDiskWizardDialog(
    ...
    private void myInit( String windir,SanBootView _view,DestAgent _host,
        ...
        actModePane = new AmsActModePaneForRestWinOrgDisk( view );
        definePane = new DefineRstRelationshipPane( view );
        selectRestDiskPane = new SelectAmsRestoreDiskPane( view,host,this,ResourceCenter.CMD_TYPE_CMDP );
...
//                    }
            ...
            if( isFirst1 ){
//                if( pane instanceof AmsActModePaneForRestWinOrgDisk ){
//                    ((AmsActModePaneForRestWinOrgDisk)pane).setAutoRst( true );
//                }
                isFirst1 = false;
            }
            refreshCenterPane();
...
SanBootView.log.error( getClass().getName(),"failed to get latest volinfo( containing UUID info ) on [ "+ host.getIP()+" / " + host.getPort()+" ]" );
 ...
 //selectRestDiskPane.getRstMappingTable();
...
//                        part = (DiskForUnix)partList.elementAt( i );
...
//                        for( j=0; j<size1; j++ ){
//                            part = (DiskForUnix)partList.elementAt(j);
...
SanBootView.log.error( getClass().getName(),"not find disk info on [ "+ ip +" / " + port+" ] " +" for "+ destdrive );
    ...
    private boolean buildRestoreMirror( Vector rstMapList ){
        ...
        for( int i=0; i<size; i++ ){
            ...
            if( srcdrive.toUpperCase().equals("C") && host.isEmBoot() ){
...
class MountOldDrive extends Thread {
    JDialog diag;
    PhyAmsRestoreOriginalDiskWizardDialog pdiag;
    ...
    public MountOldDrive( JDialog diag,PhyAmsRestoreOriginalDiskWizardDialog pdiag ){
        this.diag = diag;
        this.pdiag = pdiag;
    }
...
class PrepareWork extends Thread {
    ...
    public PrepareWork( JDialog diag,PhyAmsRestoreOriginalDiskWizardDialog pdiag,SanBootView view,DestAgent host ){
        this.diag = diag;
        this.pdiag = pdiag;
        this.view = view;
        this.host = host;
...
//            }
            ...
            boolean isOk = view.initor.mdb.getDiskPartForUnix( host.getIP(), host.getPort(), "save_partition.sh" );
            if( !isOk ){
                SanBootView.log.error( getClass().getName(), ResourceCenter.getCmdString( ResourceCenter.CMD_GET_PARTITION ) + " : " + view.initor.mdb.getErrorMessage() );
                return;
            }else{
                String diskDetailInfo = view.initor.mdb.getDiskPartForUnix();
                ...
                partList = view.initor.mdb.getIAHidenPartition2();
            }
...
//                        }else{
//                            SanBootView.log.debug( getClass().getName(),"(RestoreOriginalDiskWizardDialog::PrepareWork): "+ volMap.getVolDiskLabel() +" isn't iscsi disk" );
//                        }
//                    }
                }
            }else{
