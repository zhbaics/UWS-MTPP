...
import guisanboot.ui.InfoFromDhcpSetupPane;
import guisanboot.ui.InfoFromProtectPane;
import guisanboot.ui.InitApp;
import guisanboot.ui.InitHostable;
import guisanboot.ui.InitializingBootHost;
import guisanboot.ui.ProcessEventOnChiefHost;
import guisanboot.ui.ProgressDialog;
import guisanboot.ui.RecordUWSRptable;
import guisanboot.ui.RunningTaskPane;
import guisanboot.ui.SanBootView;
import guisanboot.ui.SelectBootHostPane;
import guisanboot.ui.SelectProtectedUnixSysVolPane;
import guisanboot.ui.SelectUnixNetCardPane;
import guisanboot.ui.SetDhcpPane;
...
import guisanboot.ams.service.CreatePPProfiel;
import guisanboot.cmdp.entity.ChiefPPProfile;
import guisanboot.ui.*;

public class AmsInitBootHostWizardDialog extends WizardDialogSample implements RecordUWSRptable,InitHostable,PhyInitInterface {
    ...
    private Map<String, Integer> mgidMap = new HashMap<String, Integer>();
    
    /** Creates new form InitNWinHostWizardDialog */
    public AmsInitBootHostWizardDialog(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
    }
    
    public AmsInitBootHostWizardDialog( SanBootView view,BootHost selHost,boolean hasEnoughSpace ){
        super( view );
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new AmsInitBootHostWizardDialog(new javax.swing.JFrame(), true).setVisible(true);
            }
        });
    }
    ...
    private void myInit( SanBootView _view,BootHost selHost,boolean hasEnoughSpace ){
        ...
        selectHostPane = new SelectBootHostPane( view,3,selHost );
        selectHostPane.setWizardDialogSample( this );
        selectSysVolPane = new SelectProtectedUnixSysVolPane( view,ResourceCenter.CMD_TYPE_CMDP );
    ...
    public Map<String, Integer> getMgidMap(){
        return mgidMap;
    }
...
//        }
        ...
        Vector selVol = this.selectSysVolPane._getVolInfo( "" );
        int size = selVol.size();
        for( int i=0; i<size; i++ ){
            BindOfUnixPartandLV binder = (BindOfUnixPartandLV)selVol.elementAt( i );
...
//                }
                ...
                if( prof == null ){
                    ...
                    //volMap = new VolumeMap( binder.tgtName,binder.part.getDiskLabel() );//binder.part.mp
                    volMap = new VolumeMap( binder.part.mp, binder.part.mp);
...
SanBootView.log.error( getClass().getName()," Modify d2d client failed: "+ip +"/" + port );                            
    ...
    @Override public void nextButtonProcess(){
        ...
        GetInitInfoForUnixThread getInitInfo;
        ...
        if( getTipTitle().compareTo(SanBootView.res.getString("InitNWinHostWizardDialog.title1").trim() ) == 0 ){
            ...
            getInitInfo = new GetInitInfoForUnixThread(
...
SanBootView.log.debug( getClass().getName()," ######### init new host: "+ ip +" / " + port );                
            ...
            if( getFirstFlag() ){ //第一次先获取所有的卷(不管孤立还是不孤立)
...
SanBootView.log.debug(getClass().getName(),"===========: "+binder.rawTgt.getCap() );
                                    ...
                                    binder.ptype = tgt.getVol_protect_type();
                                    
                                    boolean ok = view.initor.mdb.getUcsDiskInfo(binder.rawTgt.getSnap_root_id());
                                    if( ok ) {
        ...
        if( getTipTitle().compareTo(SanBootView.res.getString("InitNWinHostWizardDialog.title2").trim() ) == 0 ){
            selectSysVolPane.fireEditingStopMsgAms();
...
//            }
            ...
            if( !selectSysVolPane.hasProtectOSDiskAms(selectHostPane.cluster()) ){
                JOptionPane.showMessageDialog( this,
            ...
            bootRow = selectSysVolPane.getSelectedRowOnMpAms("/boot");
            if( bootRow != -1 && !selectHostPane.cluster()){
                if( !selectSysVolPane.isThisRowProtectedAms( bootRow ) ){
                    JOptionPane.showMessageDialog( this,
            ...
            if( this.iaHidenPartition != null ){
                ...
                for( i=0; i<size; i++ ){
                    ...
                    if( !selectSysVolPane.isThisFsProtectedAms( unixPart.mp ) ){
...
//                dupStrgyPane.setOldProfList( profList );
                ...
                //profList = this.crtPseudoLPProfile( ip,mtpp_port,aHost );
                lpprofList = this.crtPseudoLPProfile( ip, -1, aHost );
        ...
        if( getTipTitle().compareTo(SanBootView.res.getString("InitNWinHostWizardDialog.title5").trim() ) == 0 ){ 
            ...
            if( !dupStrgyPane.checkProfile(selectHostPane.cluster()) ){ return; }
...
//System.out.println("updateGUI(updateOldHost)");
    ...
    private void updateOldHostOnTree(){
        int i,size,targetID,maxSnap,protect_type,mgid;
        VolumeMap vol,tgt;
        LogicalVol lv;
...
SanBootView.log.info( getClass().getName()," ##@@@@@@@@@@@@@@@@@  Init Linux Host By Ams Begin @@@@@@@@@@@@@@@@@## ");
        
        ip = selectHostPane.getIP();
        port = selectHostPane.getPort();
        
        volInfo = selectSysVolPane._getVolInfo( ip );
        ...
        for( i=0; i<size ;i++ ){
            ...
            if( binder.isProtected ){
                if( binder.action == 0 && binder.tgtName.equals(SanBootView.res.getString("common.nprotected")) ){
                    crtList.addElement( binder );
                }
                
                //if( binder.action == 1 ){
                if( binder.tgtName.equals(SanBootView.res.getString("common.protected")) ){
...
SanBootView.log.info( getClass().getName()," ##@@@@@@@@@@@@@@@@@  Create Volume Begin @@@@@@@@@@@@@@@@@## ");         
        ...
        if( crtSize > 0 ){    
            for( i=0; i<crtSize; i++ ){
                ...
                if( binder.isProtectedByMTPP() ){ // 该卷采用mttp方式保护
    ...
    //                    }
                        ...
                        System.out.println(bksize) ;
                        long volSize = binder.part.getSizeInGiga(); // 格式化为g为单位的整型数
                        //bkNum =(int)(( tgtSize*1024*1024*1024 + bksize-1 )/bksize);
                        System.out.println(volSize) ;
                        bkNum =(int)(( volSize*1024*1024*1024 + bksize-1 )/bksize);
                        System.out.println(bkNum) ;
                        lvSize = volSize;//add by hwh

                        isOk = view.initor.mdb.addOrphVol( binder.tgtName, binder.blkSize,bkNum,binder.poolid );//TODO
                        if( isOk ){
                            runningTaskPane.setLogOnTabpane(
...
SanBootView.log.error( getClass().getName(),"tgt name space is used up. It's impossible.");
                    ...
                    bksize = ( 1<< binder.blkSize );
                    System.out.println(bksize) ;
                    long volSize = binder.part.getSizeInGiga()+1; // 格式化为g为单位的整型数
                    bkNum =(int)(( volSize*1024*1024*1024 + bksize-1 )/bksize);
                    System.out.println(volSize) ;
                    System.out.println(bksize) ;
                    lvSize = volSize;//add by hwh
                    if(binder.isucsprotected){
                        isOk = view.initor.mdb.addOrphVolAmsUcs(binder.tgtName, binder.blkSize, bkNum, binder.poolid,
                                binder.ucsOldestPoolid, binder.ucsLatestPoolid, binder.ucsLogPoolid, binder.ucsLogNumber);
                    } else {
                        isOk = view.initor.mdb.addOrphVolAms( binder.tgtName, binder.blkSize,bkNum,binder.poolid );
                    }
                    if( isOk ){
                        runningTaskPane.setLogOnTabpane(
                            SanBootView.res.getString("InitNWinHostWizardDialog.log.crtTgt") + " [ " + binder.tgtName + "(PV) ] " +
                        ...
                        if( isOk ){
                            ...
                            if( isOk ){
                                ...
                                if( isOk ){
                                    ...
                                    isOk = doLogoutTarget( ip,port, RunningTaskPane.CRT_VOL_ROW, args,iscsiVar+":"+binder.targetID );
                                    if( !isOk ){
                                        if(!selectHostPane.cluster())crtVolStepOK = false;
...
;
                    /**
                     * binder.part.mp=/mnt/test
                     * binder.part.dev_path=/dev/sdb1
                     * binder.rootID=761
                     * binder.gargetID=32781
                     * binder.lvName=vol134.mnt.tet2
                     */
                    /**
                     * parted netdisk
                     */
                    String _parted = " -I " + ResourceCenter.ISCSI_PREFIX + view.initor.mdb.getHostName() + " -S "+ view.initor.getTxIP(ip) + " -P "+ ResourceCenter.ISCSI_LOGIN_PORT + " -t " + binder.targetID ;
                    if( view.initor.mdb.loginUnixTarget( ip,port,_parted ) ){
                        runningTaskPane.setLogOnTabpane(
                            SanBootView.res.getString("InitNWinHostWizardDialog.log.loginTarget") +" [ " +":"+binder.targetID + " ] " +
                                SanBootView.res.getString("common.ok") ,
                            RunningTaskPane.CRT_VOL_ROW
                        );
                        ...
                        if( isOk ){
                            ...
                            isOk = view.initor.mdb.amsparted( ip, port, binder.part.dev_path + " " + dev);
                            if( !isOk ){
                                SanBootView.log.error(getClass().getName(), "fail to parted netdisk: " + binder.targetID + "-" + dev);
                            }
                        }else{
                            SanBootView.log.error(getClass().getName(), "fail to get dev_disk by targetid: " + binder.targetID );
                        }
                    }
                    String amsArg="";
                    if( selectHostPane.cluster() ){
                        amsArg = " -I " + ResourceCenter.ISCSI_PREFIX + view.initor.mdb.getHostName() + " -S "+ view.initor.getTxIP(ip) + " -P "+ ResourceCenter.ISCSI_LOGIN_PORT + " -t " + binder.targetID + " -d " + binder.part.dev_path + " -m " + binder.part.mp + " -r " + binder.rootID + " -C 1";
                    } else {
                        amsArg = " -I " + ResourceCenter.ISCSI_PREFIX + view.initor.mdb.getHostName() + " -S "+ view.initor.getTxIP(ip) + " -P "+ ResourceCenter.ISCSI_LOGIN_PORT + " -t " + binder.targetID + " -d " + binder.part.dev_path + " -m " + binder.part.mp + " -r " + binder.rootID + " -C 0";
                    }
                    isOk = view.initor.mdb.amsBuildMirroring(ip, port, amsArg);
                    
                    if( isOk ){
                         /**
                         * creat ams mirror MG
                         */
                        CreatePPProfiel create = new CreatePPProfiel();
                        isOk = create.createMg( view, binder, host.getID(), targetSrvName, ip, port );

                        int _mgid = view.initor.mdb.getNewId();
                        SanBootView.log.debug( getClass().getName(), "--tgtName--: " + tgtName);
                        SanBootView.log.debug( getClass().getName(), "_mgid------: " + _mgid);
                        mgidMap.put( tgtName , _mgid );

                        if( !isOk ){
                            SanBootView.log.error( getClass().getName(), "创建MG失败： " + targetSrvName);
                        }

                        runningTaskPane.setLogOnTabpane(
                            SanBootView.res.getString("InitNWinHostWizardDialog.log.amscreate") + "[" +  binder.part.mp + "-" + binder.part.dev_path + "]" + SanBootView.res.getString("common.ok"),
                            RunningTaskPane.CRT_VOL_ROW
                        );
                    }else{
                      isOk = doLogoutTarget( ip,port, RunningTaskPane.CRT_VOL_ROW, amsArg, ResourceCenter.ISCSI_PREFIX + view.initor.mdb.getHostName()+":"+binder.targetID );
                        /**
                         * 删除lunmapping并使它挂在空闲卷
                         */
                        VolumeMap vol = new VolumeMap();
                        vol.setVolDiskLabel(binder.part.mp);
                        vol.setVolTargetID(binder.targetID);
                        vol.setVol_protect_type( BootHost.PROTECT_TYPE_CMDP);
                        vol.setVol_rootid(binder.rootID);
                        vol.setVolClntID(host.getID());
                        vol.setVolName(binder.lvName);
                        binder.lv = vol;

                        isOk = delVol( host,  binder);
                        if( !isOk ){
                            SanBootView.log.error( getClass().getName(), "删除到空闲卷失败:" + "[" +  binder.part.mp + "-" + binder.part.dev_path + "]");
                        }

                        errMsg = SanBootView.res.getString("InitNWinHostWizardDialog.log.amscreate") + "[" +  binder.part.mp + "-" + binder.part.dev_path +"]" + SanBootView.res.getString("common.failed");
                        runningTaskPane.setLogOnTabpane( errMsg, RunningTaskPane.CRT_VOL_ROW );
                        writeLogBuf( errMsg, RunningTaskPane.CRT_VOL_ROW );
                        SanBootView.log.error( getClass().getName(), errMsg);
                        crtVolStepOK = false;
...
SanBootView.log.info( getClass().getName()," ##@@@@@@@@@@@@@@@@@  Save info Begin @@@@@@@@@@@@@@@@@## ");
        ...
        // 4.2 re-create profile for ams protection
SanBootView.log.info( getClass().getName()," ##@@@@@@@@@@@@@@@@@  create mj begin @@@@@@@@@@@@@@@@@## ");
        ...
        ReCrtPPProfileWhenAmsInit reCrtPPPWheninit = new ReCrtPPProfileWhenAmsInit( view,host,null,runningTaskPane,this,curVolMapList,volInfo,curNewPPPList,curPPPList,mgidMap );
        reCrtPPPWheninit.realRun();
SanBootView.log.info( getClass().getName()," ##@@@@@@@@@@@@@@@@@  create mj end @@@@@@@@@@@@@@@@@## ");
...
SanBootView.log.error( getClass().getName()," Error: not found record in dhcp config about MAC: "+ selCard.mac );                            
        ...
        String bootDirDevPath = selectSysVolPane.getBootDevPathAms(); //获取/boot对应的设备路径
...
SanBootView.log.error( getClass().getName()," Error: can't get os volume's target id.");
                ...
                Object[] val = AmsInitBootHostWizardDialog.generateIboot_cmd( tid, vg, bootMac, view.initor.getTxIP(host.getIP()), getProtectedMp( host.getID() ),
...
SanBootView.log.debug(getClass().getName()," Check lunmap if exist: ip: "+host.getIP() +"  tid: "+tgt.getVolTargetID() );                    
                    ...
                    if( isFirst ){
                        if( !withLabel ){
                             ...
                             */
                            drivepath = ResourceCenter.TARGET_TYPE + getBootTxIP()+":"+ResourceCenter.ISCSI_LOGIN_PORT+"@"+
                                    ResourceCenter.ISCSI_PREFIX+this.targetSrvName+":"+tgt.getVolTargetID();
                        }else{
                            drivepath = bkdir+"-?"+ResourceCenter.TARGET_TYPE + getBootTxIP() +":"+ResourceCenter.ISCSI_LOGIN_PORT+"@"+
                                ResourceCenter.ISCSI_PREFIX+this.targetSrvName+":"+tgt.getVolTargetID();
                        }
                        isFirst = false;
                    }else{
...
SanBootView.log.error( getClass().getName()," gen pxelinux failed. ");
    ...
    private boolean createVolMapFromCrtList( int row,Vector crtList,int cltID ){
        ...
        if( size >0 ){   
            for( int i=0; i<size; i++ ){
                binder = (BindOfUnixPartandLV)crtList.elementAt(i);
                //if( !binder.isProtectedByMTPP() ) continue; //TODO
                ...
                if( vg != null ){
                    ...
                    if( lv != null ){
                        ...
                        if( isOk ){
                            ...
                            */
                            SanBootView.log.debug( getClass().getName(), "--tgtName-----------: " + tgtName);
                            SanBootView.log.debug( getClass().getName(), "--mgidMap.size------: " + mgidMap.size());
                            if( mgidMap.containsKey( tgtName) ){
                                view.initor.mdb.modOneVolumeMap8( tgtName, mgidMap.get(tgtName) );
                            }
    ...
    private boolean createVolMapFromSelList( int row,String iscsiVar,String ip,int port,Vector selList,Vector oldVolMapList,int clntID ){
        ...
        for( i=0; i<size; i++ ){
            ...
            if( volMap.isTGT() ){
                //toDelList.addElement( volMap ); //TODO暂时去除
            }
        }
...
class GetInitInfoForUnixThread extends Thread{
    ...
    public GetInitInfoForUnixThread(  ProgressDialog pdiag, String ip,int port,boolean isGetAgtInfo,SanBootView view,boolean isFirst ){
        this.pdiag = pdiag;
        this.ip = ip;
        this.port = port;
