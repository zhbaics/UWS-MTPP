 ...
 */
public class AmsPhyFailoverWizardDialog extends WizardDialogSample implements FailoverInterface{
    ...
    public AmsPhyFailoverWizardDialog(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
    }
    
    public AmsPhyFailoverWizardDialog( SanBootView view,BootHost orgHost,boolean isSwitchDisk ){
        super( view );
...
//            }
            ...
            for( i=0; i<size; i++ ){
                ...
                //this.targetSrvName="uws727"
                //tid=32771  iscsiVar="iqn.2005-05.cn.com.odysys.iscsi.uws727:32771"
                //ip=198.88.88.18 port=5000
                //binder.volObj.vol_disklabel="/mnt/data"
                String amsArg = " -I " + ResourceCenter.ISCSI_PREFIX + view.initor.mdb.getHostName() + " -S "+ view.initor.getTxIP(ip) + " -P "+ ResourceCenter.ISCSI_LOGIN_PORT + " -t " + tid ;
                if( view.initor.mdb.loginUnixTarget( ip,port,amsArg )){
                    isOk = view.initor.mdb.amsGetRealDiskPath(ip, port ,amsArg);
                    if( isOk ){
                        String dev = view.initor.mdb.getRealDiskPath();
                        amsArg = binder.getVolMap().getVolDiskLabel() + " " + dev;
                        isOk = view.initor.mdb.amsSwitchNetDisk(ip, port, amsArg);
                        if( !isOk){
                            SanBootView.log.error(getClass().getName(), "fail to switch to netdisk:" + binder.getVolMap().getVolDiskLabel() + " " + dev);
                        }
                    }else{
                        SanBootView.log.error(getClass().getName(), "fail to get dev_disk by targetid: " + tid );
                    }
                }else{
                    SanBootView.log.error(getClass().getName(), "fail to login targetid: " + tid );
                }
...
//                        view.initor.getTxIP(), ResourceCenter.ISCSI_LOGIN_PORT+"", iscsiVar,
...
//                        isOk = view.initor.mdb.assignDriver( ip,port,
//                            view.initor.getTxIP(), ResourceCenter.ISCSI_LOGIN_PORT+"", old_part.iscsiVar,disk,ResourceCenter.CMD_TYPE_CMDP
//                        );
//                    }
//
//                    if( isOk ){
...
class CrtView extends Thread{
    AmsPhyFailoverWizardDialog diag;
