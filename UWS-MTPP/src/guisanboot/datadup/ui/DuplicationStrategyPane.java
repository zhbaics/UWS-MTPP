 ...
 */
public class DuplicationStrategyPane extends javax.swing.JPanel {
    
    /** Creates new form DuplicationStrategyPane */
    public DuplicationStrategyPane() {
        initComponents();
    }

    public DuplicationStrategyPane( SanBootView view ){
        this( view,ResourceCenter.CMD_TYPE_MTPP );
    }

    public DuplicationStrategyPane( SanBootView view,int mode ) {
        this();
        myInit( view,mode );
    }
    ...
    private void myInit( SanBootView _view,int mode ){
        ...
        jButton4.setText(SanBootView.res.getString("common.button.import"));
    ...
    //设置IP和端�?
    public void setIPAndPort( String ip,int port,int mtpp_port,String name,String machine,String os,String uuid,String bootMac,String aWinDir, String txIP ){
        this.windir = aWinDir;
    ...
    //从标签获得CMDP配置文件信息--方法内容与上述方法完全一�? 但是返回类型进行了泛�?
...
SanBootView.log.error( getClass().getName(),"failed to gen dup-hidenfs profile.");
    ...
    private void addProfile(){
        ...
        EditProfileWhenInitDialog dialog = new EditProfileWhenInitDialog( view,host,bkClnt,new ArrayList<DBSchedule>(0),null,null,bootMac,selProtectFS,hidenFs, txIP );
    ...
    private void modCMDPProfile(){
        ...
        if ( selectionline>=0 && selectionline <lineNum ){
            ...
            if( this.isCluster ){
                // 找出跟所选profile的成员相适应（相融合）的�?有disks
...
SanBootView.log.error( this.getClass().getName()," Can't find boothost or client in profile." );
            ...
            // 进去前先重新获取�?下backup object,因为备份的时候bkobj会变化，�?以只能每次都update bk obj
...
SanBootView.log.warning( getClass().getName()," no d2d bkobj, bkobj id: "+ bkobjId );
            ...
            EditProfileWhenInitDialog dialog = new EditProfileWhenInitDialog( view, host, bkClnt, schList, bkObj,prof,bootMac,selProtectFS,this.hidenFs,txIP );
            if( host.isCMDPProtect() ){
                if( this.cluster == null ){
                    dialog.setFiltered_vol_list( this.getCMDPProfileItemFromTab() );
                }else{
    ...
    public void importProfileAction() {
        if( this.cluster != null ){
            Object _subc = jComboBox1.getSelectedItem();
            if( _subc == null ){
                JOptionPane.showMessageDialog(this,
        ...
        ImportProfile2 dialog = new ImportProfile2( view,host,bkClnt,new ArrayList<DBSchedule>(0),null,null,bootMac,selProtectFS,hidenFs );
        dialog.setWizardDialogSample( wizardDiag );
