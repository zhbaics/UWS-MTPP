 ...
 */
public class ADSchedDialog extends javax.swing.JDialog {
    ...
    public static final String AUTO_DEL_SCHEDULER_PROF ="/usr/odybk/vsnap/merge_snap_util.pl";
    public static final String DEFAULT_KEEP_NUM = "8";
    ...
    /** Creates new form ADSchedDialog */
    public ADSchedDialog(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
    }
    
    /** Creates new form ADSchedDialog */
    public ADSchedDialog(java.awt.Dialog parent, boolean modal) {
        super(parent, modal);
        initComponents();
    }
    
    public ADSchedDialog( SanBootView view, DBSchedule sche,Volume vol ) {
        this( view,true );
        myInit( null,view,sche,vol );
    }
    
    public ADSchedDialog( Schedulerable diag,SanBootView view, DBSchedule sche ) {
    ...
    public static void main(String args[]) {
        new ADSchedDialog(new javax.swing.JFrame(), true).setVisible( true );
    }
...
//    private javax.swing.JComboBox jComboBox6;
    ...
    private void myInit( Schedulerable diag,SanBootView view,DBSchedule sched,Volume vol ){
        this.diag = diag;
        this.view = view;
        this.sched = sched;
...
//        setupBakLevelComboBox();
    ...
    private void setupLanguage(){
        ...
        jLabel2.setText( SanBootView.res.getString("ADSchedDialog.label.VDisk"));
        ...
        jLabel5.setText( SanBootView.res.getString("ADSchedDialog.label.count"));
...
//        jPanel3.remove( this.jComboBox6 );
    ...
    VolumeMap fixedVolumemap = null;// 只为该卷增加调度
    private void setupVolumeComboBox( Volume vol ){
        if(view.initor.mdb.updateVolumeMap()){
            Vector list = view.initor.mdb.getAllVolMaps();
            int size = list.size();

            Vector<VolumeMap> tmp = new Vector<VolumeMap>( size );
            for( int i=0; i<size; i++ ){
                VolumeMap volumemap =(VolumeMap)list.get(i); 
    //System.out.println(" profile name : \n"+profile.toString()+" indx: "+i);                      
                if( vol != null ){
                    if( volumemap.getVolName().equals(vol.getSnap_name()) ){              
                        fixedVolumemap = volumemap;             
                    }
                }
    ...
    private void  profileComboBox_actionPerformed(){
        ...
        VolumeMap volumemap = (VolumeMap)obj;
        if(volumemap != null){
           
            BootHost bhost = view.initor.mdb.getBootHostFromVector(volumemap.getVolClntID());
            if( bhost != null ){
                    jTextField2.setText( bhost.getIP() );
                    jTextField2.setToolTipText( bhost.getName() +"["+bhost.getIP()+"]" );
                }else{
...
//        jComboBox6.setEnabled( false );
    ...
    private void setupDefault(){
        if( sched == null ){ // new a scheduler
            ...
            if( fixedVolumemap != null ){
                jComboBox1.setSelectedItem( fixedVolumemap );
        ...
        if( type == CronSchedule.TYPE_DAILY_FREQ ){
            ...
            if( type == CronSchedule.TYPE_DAILY_HOUR_OCR ){
                jComboBox5.setSelectedItem(
                    SanBootView.res.getString(ADSchedDialog.TIME_UNIT_HOUR)
                );
                jSpinner3.setValue( sched.getHourFreqVal() );
            }else{
                jComboBox5.setSelectedItem(
                    SanBootView.res.getString(ADSchedDialog.TIME_UNIT_MIN)
                );
    ...
    void okButton_actionPerformed(ActionEvent e) {
        ...
        String strNum = jTextField3.getText().trim();
        if(strNum.equals("")){
            JOptionPane.showMessageDialog(this,
                SanBootView.res.getString("ADSchedDialog.errmsg.noneNumber")
            );
            return;
        }
        
//        if(Check.digitCheck(strNum)){
//            JOptionPane.showMessageDialog(this,
//                SanBootView.res.getString("ADSchedDialog.errmsg.badNumber")
//            );
//            return;
//        }
        
        int inumber = Integer.parseInt(strNum);
        if( inumber > 128 || inumber < 0 ){
            JOptionPane.showMessageDialog(this,
                SanBootView.res.getString("ADSchedDialog.errmsg.wrongNumber")
            );
            return;
        }
        ...
        if( this.displayProf ){
            Object _volumemap = jComboBox1.getSelectedItem();
            if( _volumemap == null ){
                JOptionPane.showMessageDialog(this,
                    SanBootView.res.getString("SchedDialog.errmsg.noneProfile")
                );
            ...
            volumemap = (VolumeMap)_volumemap;
        }
        tempName.append(this.AUTO_DEL_SCHEDULER_PROF).append(" ");
        tempName.append(String.valueOf(volumemap.getVol_rootid())).append(" ");
        tempName.append(strNum);
        profName = tempName.toString();
        ...
        DBSchedule dbScheduler = new DBSchedule(
            -1L,
            name,
            DBSchedule.SCH_AUTO_DEL_SNAP, // normal backup
            minute,
            hour,
            daymonth,
            month,
        ...
        if( diag!= null ){
            if( diag.isSameScheduler( dbScheduler ) ){
                JOptionPane.showMessageDialog(this,
                    SanBootView.res.getString("ADSchedDialog.errmsg.sameSched")
                );
                return;
            }
        }else{
            ...
            if( view.isSameADScheduler( dbScheduler ) ){
                JOptionPane.showMessageDialog(this,
                    SanBootView.res.getString("ADSchedDialog.errmsg.sameSched")
                );
                return;
    ...
    private String[] getDailyFreqInfo(){
        ...
        if( jRadioButton1.isSelected() ){
            ...
            if( unit.equals( SanBootView.res.getString(ADSchedDialog.TIME_UNIT_HOUR))){
                aValues[0] = "0";
                if( num == 1  ){
                    aValues[1] = "*";
                }else{
