package guisanboot.lvm.ui;

import guisanboot.ams.ui.*;
import javax.swing.*;
...
public class LvmInitBootHostWizardDialog extends WizardDialogSample implements RecordUWSRptable,InitHostable,PhyInitInterface {
    ...
    public LvmInitBootHostWizardDialog(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
    }
    
    public LvmInitBootHostWizardDialog( SanBootView view,BootHost selHost,boolean hasEnoughSpace ){
        super( view );
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new LvmInitBootHostWizardDialog(new javax.swing.JFrame(), true).setVisible(true);
            }
        });
    }
    ...
    private void myInit( SanBootView _view,BootHost selHost,boolean hasEnoughSpace ){
        ...
        selectHostPane = new SelectBootHostPane( view,4,selHost );
...
SanBootView.log.error( getClass().getName()," ##### host is not Linux. uuid: "+ uuid +" ip: "+ ip );                
               ...
               if( !BootHost.isSameHost1( getAgentInfo.getHostName() + "(LVM)", aHost.getName(), getAgentInfo.getOSName(), aHost.getOS(),
...
SanBootView.log.info( getClass().getName()," ##@@@@@@@@@@@@@@@@@  Init Linux Host By lvm Begin @@@@@@@@@@@@@@@@@## ");
                ...
                /**
                    ...
                    if( view.initor.mdb.loginUnixTarget( ip,port,_parted ) ){
                        ...
                        if( isOk ){
                            ...
                            int start = binder.part.dev_path.lastIndexOf("/");
                            int len = binder.part.dev_path.length();
                            String vg_lv = binder.part.dev_path.substring(start+1,len);
                            vg_lv = vg_lv.replace("-", "/");
                            isOk = view.initor.mdb.lvmCmd(ip, port, "crs_lv_mirror.sh " + vg_lv + " " + dev);
                            if( !isOk ){
                                SanBootView.log.error(getClass().getName(), "fail to lvm: " + binder.targetID + "-" + dev);
                            }
                        }else{
...
SanBootView.log.error( getClass().getName()," Error: can't get os volume's target id.");
                ...
                Object[] val = LvmInitBootHostWizardDialog.generateIboot_cmd( tid, vg, bootMac, view.initor.getTxIP(host.getIP()), getProtectedMp( host.getID() ),
