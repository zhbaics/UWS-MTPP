 ...
 */
public class ProcessEventOnChiefLMj extends GeneralProcessEventForSanBoot {
    BrowserTreeNode curChiefLMjNode = null;
    
    /** Creates a new instance of ProcessEventOnChiefMj */
    public ProcessEventOnChiefLMj( ){
        this( null );
    }
    public ProcessEventOnChiefLMj(SanBootView view ) {
        super( ResourceCenter.TYPE_CHIEF_LOG_MIRROR_JOB,view );
    }
    
    private void showMjList( BrowserTreeNode chiefLMjNode,int eventType ){
        curChiefLMjNode = chiefLMjNode;
        
        ChiefLogMirrorJobList chiefLMj = (ChiefLogMirrorJobList)chiefLMjNode.getUserObject();
        BrowserTreeNode fNode = chiefLMj.getFatherNode();
    ...
    public void setupTableList(){
        Object[] title = new Object[]{
            ...
            SanBootView.res.getString("View.table.mj.mgintervaltime"),
            SanBootView.res.getString("View.table.mj.mglogmaxtime"),
            SanBootView.res.getString("View.table.mj.mgmaxsnapshot"),
            SanBootView.res.getString("View.table.mj.desc")
        };
        table.setupTitle( title );
        
        int[]widthList = new int[]{
          75,120,85,55,95,160,140,130,130,170,360,160,160,140,130
        };
    ...
    public void insertSomethingToTable( Object obj ){
        ...
        mj.mj.setFatherNode( curChiefLMjNode );
        ...
        GeneralBrowserTableCell mgMITCell,mgLMTCell,mgMSSCell;
        if(mj.mj.getMg_interval_time() > 0){
            mgMITCell = new GeneralBrowserTableCell(-1,mj.mj.getMg_interval_time()/60+"",JLabel.LEFT);
        } else {
            mgMITCell = new GeneralBrowserTableCell(-1,"",JLabel.LEFT);
        }
        if(mj.mj.getMg_log_max_time() > 0){
            mgLMTCell = new GeneralBrowserTableCell(-1,mj.mj.getMg_log_max_time()/60+"",JLabel.LEFT);
        } else {
            mgLMTCell = new GeneralBrowserTableCell(-1,"",JLabel.LEFT);
        }
        if(mj.mj.getMg_max_snapshot() > 0){
            mgMSSCell = new GeneralBrowserTableCell(-1,mj.mj.getMg_max_snapshot()+"",JLabel.LEFT);
        } else {
            mgMSSCell = new GeneralBrowserTableCell(-1,"",JLabel.LEFT);
        }
