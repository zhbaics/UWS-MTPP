 ...
 */
public class SelectUnixVolDialog extends javax.swing.JDialog {
    
    /** Creates new form SelectVolDialog */
    public SelectUnixVolDialog(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
    }
    
    public SelectUnixVolDialog(java.awt.Dialog parent, boolean modal) {
        super(parent, modal);
        initComponents();
    }
    
    public SelectUnixVolDialog( JDialog diag, SanBootView view,PaneEditorable tab ){
        this( diag,true );
        myInit( view,tab );
    }
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        ...
        if( volMap != null ){
            // 该volMap一定是个TGT类型的volumeMap
            VolumeMap lv = view.initor.mdb.getLVOnClntAndVG( volMap.getVolClntID(), volMap.getVolDiskLabel() );
            VolumeMap vg = view.initor.mdb.getVGOnVGName( volMap.getVolDiskLabel(), volMap.getVolClntID() );
            
            //表示当前所选的是个lv
            values = new Object[4];
            values[0] = tgt;
            values[1] = lv; 
            values[2] = vg.getVolDiskLabel();  // lvmType
            values[3] = vg.getMaxSnapNum()+""; // snapSpace
        }else{
            // 表示当前所选的是个纯粹的orph vol(初始化windows时产生的orhp vol或者初始化linux时产生的orph vol/vg/lv) 
            values = new Object[1];
            values[0] = tgt;
        }
    ...
    private void myInit( SanBootView _view,PaneEditorable _tab ){
        view = _view;
        tab = _tab;
