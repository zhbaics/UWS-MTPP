/*
 * RunningTaskForRestoreLocalDisk.java
 *
 ...
 */
public class RunningTaskForRestoreLocalDisk extends javax.swing.JPanel implements RunningTask {
    /** Creates new form RunningTaskForRestoreLocalDisk */
    public RunningTaskForRestoreLocalDisk() {
        initComponents();
    }
    
    public RunningTaskForRestoreLocalDisk( SanBootView view,DestAgent host ) {
        this();
        myInit( view,host );
    }
    ...
    public void updateGUI1( Vector rstList ){
        ...
        for( i=0; i<size; i++ ){
            ...
            scroPaneArray[i].setHorizontalScrollBarPolicy( JScrollPane.HORIZONTAL_SCROLLBAR_ALWAYS  );
...
System.out.println( "rst seq: " + seqList );
        ...
        while( iterator.hasNext() ){
            ...
            dest1="";
            size = volList.size();
            ptype = BootHost.PROTECT_TYPE_CMDP;
            for( i=0; i<size; i++ ){
                binder = (BindOfVolMapandDest)volList.elementAt( i );
                ptype = binder.volMap.getVol_protect_type();
                if( isFirst ){
    ...
    public void freshCopyLogOnTabpane( String prefix,String log,int which,boolean isAutoScroll ){
        ...
        for( int i=0; i<lines.length; i++ ){
            ...
            textAreaArray[which].append( lines[i] );
            textAreaArray[which].append( "\n" );
            textAreaArray[which].setPreferredSize( new Dimension( 0,vertical_base+=15 ) );
            textAreaArray[which].getCaret().setDot( textAreaArray[which].getText().length() );
            scroPaneArray[which].scrollRectToVisible( textAreaArray[which].getVisibleRect() );
