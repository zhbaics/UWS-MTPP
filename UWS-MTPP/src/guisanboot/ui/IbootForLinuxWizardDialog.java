/*
 * IbootForLinuxWizardDialog.java
 *
 * Created on 2009/6/22,AM 9:51
 */
 ...
 */
public class IbootForLinuxWizardDialog extends WizardDialogSample implements FailoverInterface,UnixIbootable{
    SelectHostForNetBootPane selectHostForNetBoot;
    SelectNetCardPane selectNetCardPane;
    SetDhcpPane setDhcpPane;
    SelectUnixSnapshotPane selectSnapPane;
    ...
    /** Creates new form IbootForLinuxWizardDialog */
    public IbootForLinuxWizardDialog(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
    }
    
    public IbootForLinuxWizardDialog( SanBootView view,BootHost orgHost,String targetSrvName,Vector partList,String original_boot_MAC ){
        super( view );
        myInit( view,orgHost,targetSrvName,partList,original_boot_MAC );
    }
    ...
    private void myInit( SanBootView _view,BootHost _host,String _tgtSrvName,Vector _partitionList,String _original_boot_MAC ){
        view = _view;
        host = _host;
        ...
        selectSnapPane = new SelectUnixSnapshotPane( view );
    }
    ...
    @Override public void nextButtonProcess(){
        boolean isOk,contained,isWin,isUseOdyDhcp=true;
        int hostID;
        ...
        if( getTipTitle().compareTo( SanBootView.res.getString("IbootForLinuxWizardDialog.title1").trim() ) == 0 ){
            ...
            if( netBootedHost == null ){
                ...
                if( !contained ){
                    hostID = netBootedHost.getID();
                    isWin = netBootedHost.isWinHost();
                    if( isWin ){
                    ...
                    if( host.getID() == netBootedHost.getID() ){
                        // 要网络启动的物理主机就是初始化的原机, 获取orginal host在初始化时所选的启动网卡
...
SanBootView.log.debug( getClass().getName(),"selected netboot MAC: "+ mac ); 
        ...
        if( getTipTitle().compareTo( SanBootView.res.getString("IbootForLinuxWizardDialog.title3").trim() ) == 0 ){
            ...
            // 将版本准备好
            if( ( netBootedHost != null ) && ( netBootedHost.getID() == host.getID() ) ){
                // 进行网络启动的物理机器就是发生初始化的原机
                contained = oldBootVer.containsKey( host.getIP() );
                if( !contained ){
                    oldBootVerList = new BootVerList( view.getUnixBootVer( host.getID() ),null );
            ...
            if( bindList == null ){
                ...
                GetUnixRstVer getRstVer = new GetUnixRstVer( initDiag,view,host.getID() );
                getRstVer.start();
                initDiag.mySetSize();
        ...
        if( getTipTitle().compareTo( SanBootView.res.getString("IbootForLinuxWizardDialog.title4").trim() ) == 0 ){
            ...
            CrtUnixView crtView = new CrtUnixView( 
                this,
                initDiag,
                selectSnapPane.getSelectedSnap1().toArray(),
                viewMap,
                view,
                bootMac,
                host,
            ...
            if( crtView.allFinish ){
                selVer_info = crtView.getSelVersionInfo();
