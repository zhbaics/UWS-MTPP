/*
 * PhyFailoverWinClusterWizardDialog.java
 *
 ...
 */
public class PhyFailoverWinClusterWizardDialog extends WizardDialogSample implements FailoverInterface{
    ...
    public PhyFailoverWinClusterWizardDialog(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
    }
    
    public PhyFailoverWinClusterWizardDialog( SanBootView view,Cluster orgCluster,boolean isSwitchDisk ){
        super( view );
        myInit( view,orgCluster,isSwitchDisk );
    }
    ...
    private void myInit( SanBootView _view,Cluster _clus,boolean isSwitchDisk ){
        view = _view;
        cluster = _clus;
        ...
        seleDRReoverMode.setAutoRebootFlag( cluster.isAutoReboot() );
        if( cluster.isAutoDR() ){
            seleDRReoverMode.setAutoMode( true );
        }else{
            seleDRReoverMode.setManualMode( true );
        }
        selectHostForNetBoot = new SelectHostForNetBootPane( view,cluster );
        ...
        selectSnapPane.setCluster( cluster );
    ...
    /*
    private ArrayList getNetCardList( BootHost selHost ){
        int i,j,size,size1;
        ArrayList netCardList;
        boolean isOk;

        ArrayList<SubCluster> subcList = cluster.getRealSubCluster();
        ArrayList<NetCard> ret = new ArrayList();
        size = subcList.size();
        for( i=0; i<size; i++ ){
            SubCluster subc = subcList.get(i);
            BootHost aHost = subc.getHost();
            if( selHost != null ){
                if( !selHost.getUUID().equals( aHost.getUUID() ) ){
                    continue;
                }
            }

            isOk = view.initor.mdb.getIPInfoFromSrv( ResourceCenter.CLT_IP_CONF+"/"+ aHost.getID() + ResourceCenter.CONF_IP );
            if( isOk ){
                 netCardList = view.initor.mdb.getAllNetCardinfoFromSrv();
                 size1 = netCardList.size();
                 for( j=0; j<size1; j++ ){
                     NetCard nc = (NetCard)netCardList.get( j );
                     nc.host_ip = aHost.getIP();
                     ret.add( nc );
                 }
            }else{
                JOptionPane.showMessageDialog(this,
    ...
    private void getServiceForCMDP( HashMap map ){
         ArrayList<PPProfileItem> itemList = view.initor.mdb.getPPProfileItem1( cluster.getCluster_id() );
    ...
    private void setupServiceTable(){
        ...
        if( servList == null ){ // 先将server table布置好
            ...
            for( int i=0; i<size; i++ ){
                // 获取集群上的所有节点的所有服务（ 其实应该获取所选C盘对应的物理主机的服务 ）
                BootHost aHost = subcList.get(i).getHost();
                isOk = view.initor.mdb.getOSService( aHost.getIP(),aHost.getPort(),"",ResourceCenter.CMD_TYPE_CMDP );
                if( isOk ){
                    this.addService( map, view.initor.mdb.getOSService() );
                }
                // 再获取volMap上绑定的服务（它们没有描述和路径）
                this.addService( map, view.initor.mdb.getServiceOfVolMapOnClntID( aHost.getID() ) );
            }
            
            // 再获取服务器上保存的
            isOk = view.initor.mdb.getOSService( ResourceCenter.CLT_IP_CONF+"/cluster-"+cluster.getCluster_id() + ResourceCenter.CONF_SERVICE,ResourceCenter.CMD_TYPE_CMDP );
            if( isOk ){
                this.addService(map, view.initor.mdb.getOSService() );
            }
            
            if( !cluster.isStopAllBaseServ() ){
    ...
    private String getBootedMacForCluster( ArrayList netCardList ){
        String ret = "";
        boolean isOk = view.initor.mdb.getNetInfoFromMDB( ResourceCenter.CLT_IP_CONF+"/"+netBootedHost.getID() + ResourceCenter.CONF_IP );
        if( isOk ){
            ret = view.initor.mdb.getBootMac();
        }
        if( ret.equals("") ){
            NetCard nc = getOneNc( netBootedHost.getIP(),netCardList );
            if( nc != null ){
                ret = nc.mac;
            }
        }
        ...
        if( getTipTitle().compareTo(SanBootView.res.getString("InitBootHostWizardDialog.selNetBootMode").trim() ) == 0 ){
            if( selectNetBootModePane.isSelectIScsiHBAMode() ){
                ...
                if( ( netBootedHost != null ) && ( netBootedHost.getID() == host.getID() ) ){
                    ...
                    }else{ // select one host (but not the original host)
                ...
                if( netBootedHost == null ){
                    ...
                    if( !contained ){
                        ...
                        if( cluster.isBelongedThisCluster( netBootedHost ) ){
                            // 要网络启动的物理主机就是初始化的原机, 获取orghost在初始化时所选的启动网卡
                            isOk = view.initor.mdb.getNetInfoFromMDB( ResourceCenter.CLT_IP_CONF+"/"+netBootedHost.getID() + ResourceCenter.CONF_IP );
                            if( isOk ){
...
SanBootView.log.debug( getClass().getName(),"selected netboot MAC: "+ mac ); 
                    start_ip = this.cluster.getOneRealHost().getIP();
...
SanBootView.log.debug( getClass().getName(),"selected netboot MAC: "+ mac ); 
                    ...
                    if( cluster.isBelongedThisCluster( netBootedHost ) ){
                        ...
                        // get the 3rd dhcp info
                        isOk = view.initor.mdb.get3rdDhcpInfo( ResourceCenter.CLT_IP_CONF+"/"+netBootedHost.getID()+ ResourceCenter.CONF_3RD_DHCP );
                        if( isOk ){
                            ip_3rd = view.initor.mdb.getIpFrom3rdDhcpInfo();
                            nextsrv_3rd = view.initor.mdb.getNextServerFrom3rdDhcpInfo();
                        }
        ...
        if( getTipTitle().compareTo(SanBootView.res.getString("FailoverWizardDialog.title2.6").trim() ) == 0 ){
            ...
            if( tgtList == null ){
                tgtList = view.initor.mdb.getVolFromCluster1( this.cluster.getCluster_id() );
            }
            
            // 将版本准备好
            if( ( netBootedHost != null ) && cluster.isBelongedThisCluster( netBootedHost ) ){
                // 进行网络启动的物理机器就是发生初始化的原机��ԭ��
                contained = oldBootVer.containsKey( cluster.getClusterIP() );
                if( !contained ){
                    oldBootVerList = view.getBootVerForWinCluster( cluster.getCluster_id() );
                    oldBootVer.put( cluster.getClusterIP(), oldBootVerList );
                }else{
                    oldBootVerList = (BootVerList)oldBootVer.get( cluster.getClusterIP() ) ;
                }
            }else{
                // 进行网络启动的是其他主机或是指定的网卡������
                DestAgent da = getSelectedMC( bootMac );
                if( da != null ){
                    ...
                    contained = oldBootVer.containsKey( bootMac );
                    if( !contained ){
                        oldBootVerList = view.getBootVerForDefaultForWinCluster( cluster.getCluster_id() );
                        oldBootVer.put( bootMac,oldBootVerList );
                    }else{
                        oldBootVerList = (BootVerList)oldBootVer.get( bootMac );
                    }
            ...
            if( bindList == null ){
                ...
                GetRstVersion getRstVer = new GetRstVersion( initDiag,view,-1,cluster.getCluster_id(),true,true,false );
                getRstVer.start();
                initDiag.mySetSize();
                initDiag.setLocation( view.getCenterPoint( initDiag.getDefWidth(),initDiag.getDefHeight() ) );
                initDiag.setVisible( true );
        ...
        if( getTipTitle().compareTo(SanBootView.res.getString("FailoverWizardDialog.title3").trim() ) == 0 ){
            ...
            if( ( netBootedHost == null ) || !cluster.isBelongedThisCluster( netBootedHost ) ){
                ...
                if( !selectSnapPane.multiFsToRestore1() ){
                    JOptionPane.showMessageDialog(this,
                        SanBootView.res.getString("FailoverWizardDialog.error.multiFsToRst")
                    );
                    return;
...
SanBootView.log.error( getClass().getName(),"target server name is null." );                    
            ...
            CrtView crtView = new CrtView( 
                ...
                viewMap,view,bootMac,cluster,netBootedHost,
                start_ip,targetSrvName,
                ( netBootedHost != null ) && cluster.isBelongedThisCluster( netBootedHost ),
...
SanBootView.log.info( getClass().getName()," @@#################  Migration System from local disk to iSCSI Begin ###################@@ ");
        if( this.isSwitchDisk ){
            ip   = this.netBootedHost.getIP();
            port = this.netBootedHost.getPort();

            host = this.netBootedHost;
        }else{
            // 获取指定恢复的C盘对应的主机,该主机实际代表了网络启动的原机 （2011.8.4）
            host = (BootHost)this.selectSnapPane.getHostInfoAboutOSDisk();
...
SanBootView.log.info( getClass().getName()," @@#################  Connect Agent End ###################@@ ");
        ...
        if( isOk ){
            if( view.initor.mdb.isStartupFromSAN() ){
                ...
                if( host.isLargerThanVista() || cluster.isIBoot() || cluster.isISCSIHBABoot() ){
...
SanBootView.log.info( getClass().getName()," @@#################  Connect Agent End ###################@@ ");             
        ...
        if( this.isRealNetBootOnLocalHost ){
            Vector volOnClnt = view.initor.mdb.getVolFromCluster1( cluster.getCluster_id() );
            ...
            if( !this.isSwitchDisk ){
                ...
                for( i=0; i<size; i++ ){
                    ...
                    if( tid1 == -1 ){ // local disk
                        ...
                        isOk = view.initor.mdb.assignDriver( ip,port,
                            view.initor.getTxIP(ip), ResourceCenter.ISCSI_LOGIN_PORT+"", old_part.iscsiVar,disk,ResourceCenter.CMD_TYPE_CMDP
                        );
                    }
...
SanBootView.log.info( getClass().getName()," @@#################  Migration system from local disk to iSCSI End ###################@@ ");        
    ...
    private DestAgent getSelectedMC( ArrayList netCardList ){
        ...
        ArrayList netBootedHostList = view.getNetbootedHostOnHost( cluster.getTreeNode() );
        size = netCardList.size();
        for( i=0; i<size; i++ ){
...
class CrtView extends Thread{
    PhyFailoverWinClusterWizardDialog diag;
    ...
    private String getTimeStamp( String diskLabel ){
        ...
        while( iterator.hasNext() ){
            ...
            isSame = view.initor.mdb.isBelongedToSameDg1( cluster.getCluster_id(), key, diskLabel );
            if( !isSame ) continue;
...
System.out.println(" bootMac: "+ bootMac +" bootMacForUnix: "+ bootMacForUnix + " bootMacForWin: "+ bootMacForWin +" simpleMac: "+simpleMac );
        ...
        for( i=0; i<size; i++ ){
            ...
            if( letter.toUpperCase().equals("C") && bindvs.isSel ){
                osvs = bindvs;
            }
                ...
                if( cluster.isEmBoot() ){
                    ...
                    if( isOk ){
                        if( view.initor.mdb.getFromVectorOnMac( simpleMac ) != null ){
                            ...
                            isOk = view.initor.mdb.addIboot( simpleMac, view.initor.getTxIP(cluster.getClusterIP()), osTid );
                        }else{
                            isOk = view.initor.mdb.addIboot( simpleMac, view.initor.getTxIP(cluster.getClusterIP()), osTid );
                        }
...
SanBootView.log.error( getClass().getName(),"iboot option is bad : "+ simpleMac );                                    
        ...
        if( allFinish && !isSkip ){
            boolean useOdyDhcp = !( cluster.getCluster_boot_mode() == BootHost.BOOT_MODE_ISCSI_HBA ) && diag.setDhcpPane.isAutoSetup();
...
SanBootView.log.error( getClass().getName(),"Error: not found record in dhcp config about MAC: " + bootMacForUnix );  
                                ...
                                if( isNetBootOnLocalHost ){
                                    if( !cluster.isUseOdyDhcp() ){
                                        cluster.setUseOdyDhcp();
                                        view.initor.mdb.modOneCluster2( cluster.getCluster_id(),cluster.getCluster_stop_base_service() );
                                    }
                                }else{ // is a destagent
                                    //view.initor.mdb.delFile( ResourceCenter.CLT_IP_CONF + "/"+ResourceCenter.PREFIX_DST_AGNT + da_id + ResourceCenter.CONF_3RD_DHCP );
                                }
                ...
                if( isNetBootOnLocalHost ){
                    ...
                    String conf = ResourceCenter.CLT_IP_CONF + "/cluster-" + cluster.getCluster_id() + ResourceCenter.CONF_3RD_DHCP;
...
SanBootView.log.error(getClass().getName(), "failed to get relatived boothost with bootMac: " + bootMac );
                ...
                for( int aI=0; aI<aSize; aI++ ){
                    BootHost aNetBootedHost = view.initor.mdb.getBootHostFromVector( id_list.get(aI).intValue() );
                    if( aNetBootedHost == null ) continue;
                    if( cluster.isBelongedThisCluster1( aNetBootedHost ) ){
                        realNetBootOnLocalHost = true;
