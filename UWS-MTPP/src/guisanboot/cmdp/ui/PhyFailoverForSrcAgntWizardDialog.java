/*
 * PhyFailoverForSrcAgntWizardDialog.java
 *
 * Created on 2010/7/19�, PM 6:09
 */
 ...
 */
public class PhyFailoverForSrcAgntWizardDialog extends WizardDialogSample implements FailoverInterface {
    ...
    /** Creates new form PhyFailoverForSrcAgntWizardDialog */
    public PhyFailoverForSrcAgntWizardDialog(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
    }
    
    public PhyFailoverForSrcAgntWizardDialog( SanBootView view,SourceAgent orgHost,boolean mustCheckDiskExist  ){
        super( view );
    ...
    private void myInit( SanBootView _view,SourceAgent _host,boolean _mustCheckDiskExist ){
        ...
        runPane = new RunningTaskForDRPane( view,ResourceCenter.CMD_TYPE_CMDP,false );
    }
    ...
    @Override public void nextButtonProcess(){
        ...
        if( getTipTitle().compareTo( SanBootView.res.getString("FailoverWizardDialog.title1").trim() ) == 0 ){
            ...
            if( servList == null ){ // 不管是否显示"select service pane",先将server table布置好
                ...
                isOk = view.initor.mdb.getOSService(
                    ResourceCenter.CLT_IP_CONF+"/"+ ResourceCenter.PREFIX_SRC_AGNT + host.getSrc_agnt_id() + ResourceCenter.CONF_SERVICE,
        ...
        if( getTipTitle().compareTo(SanBootView.res.getString("InitBootHostWizardDialog.selNetBootMode").trim() ) == 0 ){
            if( selectNetBootModePane.isSelectIScsiHBAMode() ){
                ...
                if( bindList == null ){
                    ...
                    GetUnixRstVerForSrcAgent getRstVer = new GetUnixRstVerForSrcAgent( initDiag,view,host,mustCheckDiskExist,true );
                    getRstVer.start();
                    initDiag.mySetSize();
...
SanBootView.log.error( getClass().getName(),"target server name is null." );                    
            ...
            CrtView1 crtView = new CrtView1( this,initDiag,selectSnapPane.getSelectedSnap().toArray(),
                viewMap,view,bootMac,host,netBootedHost,start_ip,targetSrvName,false,false
            );
            crtView.start();
            initDiag.mySetSize();
            initDiag.setLocation( view.getCenterPoint( initDiag.getDefWidth(),initDiag.getDefHeight() ) );
...
SanBootView.log.error(getClass().getName(), "failed to destory mirror on " + drvLetter + " for [ "+ ip+"/"+port+" ], so assign-driver may be failed.");
                ...
                if( isOk ){
                    runPane.setLogOnTabpane(
                        SanBootView.res.getString("FailoverWizardDialog.log.assignDriver") +" [ "+iscsiVar+" -> "+drvLetter+" ] "+
                            SanBootView.res.getString("common.ok"), 
                        RunningTaskForDRPane.ASSIGN_DRIVER_ROW
                    );
           ...
           // 将当前不需要的target的lunmap去掉
            ...
            for( i=0; i<size; i++ ){
                ...
                if( tid1 == -1 ){ // local disk
                    if( part.getDiskLabel().toUpperCase().equals("C:\\") ){
                        ...
                        // 执行umount命令�
                        isOk = view.initor.mdb.assignDriver( ip,port, "NULL","NULL",part.volInfo,"0",ResourceCenter.CMD_TYPE_CMDP );
                        if( isOk ){
                            runPane.setLogOnTabpane(
                        ...
                        isOk = view.initor.mdb.assignDriver( ip,port, "NULL","NULL",part.volInfo,disk, ResourceCenter.CMD_TYPE_CMDP );
                    }
                }else{ // iscsi target volume
                    isOk = view.initor.mdb.assignDriver( ip,port, 
                        view.initor.getTxIP(ip), ResourceCenter.ISCSI_LOGIN_PORT+"", part.iscsiVar,disk,ResourceCenter.CMD_TYPE_CMDP
                    );
                }
            ...
            // 记录最新的恢复版本(对boothost而言是:修改volumemap中的vol_view_targetid;
            // 对destagent而言是修改snapusage)
            SnapUsage su = null;
            int da_id = -1;
            int selVol_rootid = -1;
...
SanBootView.log.info( getClass().getName()," @@#################  Start Service End ###################@@ ");
        ...
        // 判断是否全部成功
        allFinished = ( stopServStepOK && loadInfoStepOK && assignDriverStepOK && startServStepOK );
...
class CrtView1 extends Thread{
    PhyFailoverForSrcAgntWizardDialog diag;
                        ...
                        if( bindsv.snap instanceof Snapshot ){
                            ...
                            if( tmpsnap.getSnap_root_id() == selSnap.getSnap_root_id() &&
                                ...
                                if( !isOk ){
                                    ...
                                    if( !isOk ){
SanBootView.log.error(getClass().getName(), "failed to zero uuid of sector for view: "+ newView.getSnap_root_id()+"/"+newView.getSnap_local_snapid() );
                                        JOptionPane.showMessageDialog(pdiag,
                                            SanBootView.res.getString("FailoverWizardDialog.error.crtView")+" : [ SnapID: "+selSnap.getCreateTimeStr()+" ]"
                                        );
...
SanBootView.log.error( getClass().getName(),"Error: not found record in dhcp config about MAC: " + bootMacForUnix );  
        ...
        if( allFinish ){
            // 将netbootedhost.clnt_auto_switch_disk置为0，这样程连武自动磁盘切换将失效
            if( netBootedHost != null ){
