 ...
 * @author zjj
 */
public class EditPhyProfileDialog extends javax.swing.JDialog {
    /** Creates new form EditPhyProfileDialog */
    public EditPhyProfileDialog(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
    }

    public EditPhyProfileDialog( SanBootView view,
        PPProfile ppprof,
        Vector servicesList
    ){
        this( view,true );
        myInit( view,ppprof,servicesList );
    }
    ...
    private void initComponents() {
        ...
        jTextField4.setPreferredSize(new java.awt.Dimension(80, 21));
        gridBagConstraints = new java.awt.GridBagConstraints();
    ...
    */
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                EditPhyProfileDialog dialog = new EditPhyProfileDialog(new javax.swing.JFrame(), true);
    ...
    private void myInit( SanBootView view,PPProfile ppprof,Vector services ){
        this.view = view;
        this.ppprof = ppprof;
    ...
    private void setLanguage(){
     ...
     //   this.jButton6.setText(SanBootView.res.getString("common.button.add") );
        this.jButton7.setText(SanBootView.res.getString("common.button.mod") );
     //   this.jButton8.setText(SanBootView.res.getString("common.button.del") );
...
SanBootView.log.warning( getClass().getName(), "don't exist the service on host, it's name: " + services );
    ...
    private PPProfileItem mainDisk = null;
    private void initUI( PPProfile ppprof ){
        if( ppprof == null ) return;
        this.setupDiskPane( ppprof.getElements() );

        mainDisk = ppprof.getMainDiskItem();
        if( mainDisk != null ){
            jTextField4.setText( mainDisk.getMg().getMg_min_snap_size()+"" );
            jTextField5.setText( mainDisk.getMg().getMg_max_snapshot()+"" );

            if( mainDisk.getMg().isRotateType() ){
                Integer[] itemOfinterval = ppprof.getItemValOfIntervalTime();
                ...
                this.jTextArea1.setText( mainDisk.getMg().getCronSchString() );
...
SanBootView.log.debug( getClass().getName(), " db_type: "+ db_type );
            ...
            jTextField6.setText( mainDisk.getVolMap().getDatabase_instances() );
            
            this.setupServicePane( mainDisk.getVolMap().getServices() );

            this.jCheckBox1.setSelected( mainDisk.getVolMap().isAutoAsyncAftCrtSnap() );
        }
    }
    ...
    private void do_editCron(){
        EditCronSchDialog dialog = new EditCronSchDialog( view,mainDisk.getMg() );
        ...
        this.mainDisk.getMg().setMg_schedule_minute( (String)ret[0] );
        this.mainDisk.getMg().setMg_schedule_hour1( (String)ret[1] );
        this.mainDisk.getMg().setMg_schedule_day( (String)ret[2] );
        this.mainDisk.getMg().setMg_schedule_month( (String)ret[3] );
        this.mainDisk.getMg().setMg_schedule_week( (String)ret[4] );
        this.mainDisk.getMg().setMg_schedule_clock_zone( (String)ret[5] );
        this.mainDisk.getMg().setMg_schedule_hour( (String)ret[6] );
        this.mainDisk.getMg().setMg_schedule_clock_set( (String)ret[7] );
        this.mainDisk.getMg().setMg_schedule_type( ((Integer)ret[8]).intValue() );

        this.jTextArea1.setText( this.mainDisk.getMg().getCronSchString() );
    }
    ...
    private void do_ModDisk(){
        ArrayList<PPProfileItem> allDisks = view.initor.mdb.getPPProfileItem( ppprof.getHostID() );
    ...
    private boolean checkValid(){
        ...
        if( this.jRadioButton1.isSelected() ){
            ...
            sch_type = this.mainDisk.getMg().getMg_schedule_type();
        }
...
//            }
        ...
        String instances = this.jTextField6.getText().trim();
        ArrayList<Service> selServices = this.getSelServiceOnGUI();
        this.dispose();

        EditPPProfileThread thread = new EditPPProfileThread(
            view,
            this.ppprof,
            selDisks,
            sch_type,
        ...
        view.startupProcessDiag(
            SanBootView.res.getString("View.pdiagTitle.editPPProf"),
            SanBootView.res.getString("View.pdiagTip.editPPProf"),
            thread
        );
    }
}
