...
public class UcsForwordView extends NetworkRunning{
...
SanBootView.log.debug( getClass().getName(), "(AddView): "+ s1 );
    ...
    public UcsForwordView( String cmd,Socket socket) throws IOException {
        super( cmd,socket );
    }
    
    public UcsForwordView( String cmd ){
        super( cmd );
    }

    public boolean realDo( int rootid,int snapid,int IOnum){
        String aCmdLine = ResourceCenter.getCmd( ResourceCenter.CMD_FORWORD_UCS_VIEW ) +
                "-n " + IOnum + " " + rootid + " "  + snapid ;
...
SanBootView.log.error( getClass().getName()," add view cmd errmsg:"+ this.getErrMsg()  );
    ...
    public boolean realDo( int rootid,int snapid,String timestamp ){
        String aCmdLine = ResourceCenter.getCmd( ResourceCenter.CMD_FORWORD_UCS_VIEW )
                +( timestamp.equals("")?"":" -t " + timestamp + " " ) + rootid + " "  + snapid ;
        this.setCmdLine( aCmdLine );
SanBootView.log.info( getClass().getName()," add view cmd:"+ this.getCmdLine() );
