/*
 * StartorStopUnlimitedIncMjThread.java
 *
 * Created on 2009/12/15, PM�3:11
 *
 ...
 * @author zjj
 */
public class StartorStopUnlimitedIncMjThread extends BasicStartorStopMjThread{ 
    /** Creates a new instance of StartorStopUnlimitedIncMjThread */
    public StartorStopUnlimitedIncMjThread(
        SanBootView view,
        MirrorJob mj,
        int action,
        Object host,
    ...
    @Override public boolean realRun(){
        ...
        isLocalType = mj.isLocalIncMjType();
...
SanBootView.log.info( getClass().getName(), "get src-agent according uuid: "+ uuid + " and uws server id: "+src_uws.getUws_id() );                                    
                ...
                if( host != null ){
                    MirrorDiskInfo mdi = view.initor.mdb.getMDIFromCacheOnRootID( dest_rootid );
                    int host_id = ( (host instanceof BootHost )? ((BootHost)host).getID() : ((SourceAgent)host).getSrc_agnt_id() );
                    if( mdi == null ){
                        isOk = crtMDI( dest_rootid, host_id, src_rootid, vol_name,vol_mp );
                    }else{
                        if( mdi.getSrc_agnt_id() != host_id ){
                            SourceAgent aSa = view.initor.mdb.getSrcAgntFromVectorOnID( mdi.getSrc_agnt_id() );
                            if( ( aSa != null ) && aSa.isRollbackedHost() ){
                                isOk = false;
                                errMsg =  SanBootView.res.getString("MenuAndBtnCenter.error.prepareJobFail1");
                            }else{
                                isOk = modMDI( dest_rootid, host_id );
                            }
                        }
                    }
                }else{
                    UWSrvNode src_uws = view.initor.mdb.getUWSrvFromVector ( src_uws_psn );
                    if( src_uws == null ){
                        src_uws = new UWSrvNode( -1, view.initor.getTxIP(dest_uws_ip), UWSrvNode.MEDIA_SERVER_PORT, src_uws_psn );
                        isOk = view.initor.mdb.addUWSrv( "", dest_uws_port, dest_poolid,dest_pool_passwd,src_uws );
                        if( !isOk ){
                            errMsg =  SanBootView.res.getString("MenuAndBtnCenter.error.prepareJobFail");
                        }else{
                            src_uws.setUws_id( view.initor.mdb.getNewId() );
                    ...
                    if( isOk ){
                        SourceAgent tmpSrcAgnt = view.initor.mdb.getSrcAgntForSrcUWSOnPSN( src_uws_psn );
                        if( tmpSrcAgnt == null ){
                            // 代表源端UWS的srcagnt的uuid为空,src_agent_os字段表示psn
                            isOk = crtSrcAgent( view.initor.getTxIP(dest_uws_ip), src_uws_psn, "", "",src_uws.getUws_id(), -1, -1, 0, 1 );
                        }else{
...
System.out.println(" linux boot mac: " + linuxOrgBootMac );    
                        ...
                        if( !dest_uws_tftproot.equals("") && !src_uws_tftproot.equals("") ){
                            this.isOkForSyncManageInfo = this.transferManageInfo( src_uws_tftproot, dest_uws_tftproot, orgSimpleMac, srcAgnt.getSrc_agnt_id(),false );
